...
def create_radar_chart(first_level_scores, first_weights, top_n=10):
    """
    为前N名动漫创建雷达图
    ...
    final_scores['anime'] = first_level_scores['anime']
    
    final_score = np.zeros(len(first_level_scores))
    for first_code, weight in first_weights.items():
        final_score += first_level_scores[first_code].values * weight
    ...
    for i, (idx, row) in enumerate(top_animes.iterrows()):
        ...
        for first_code in config.FIRST_LEVEL_NAMES.keys():
            if first_code in first_level_scores.columns:
                values.append(row[first_code])
            else:
                values.append(0)
    ...
    # 保存图片
    plt.tight_layout()
    plt.savefig('output/top_animes_radar.png', dpi=300, bbox_inches='tight')
    plt.savefig('output/top_animes_radar.pdf', bbox_inches='tight')
    plt.show()
    
    print(f"雷达图已保存到 output/top_animes_radar.png 和 output/top_animes_radar.pdf")
...
def generate_detailed_report(final_scores, first_level_scores, second_level_scores, 
    ...
    first_level_scores: 一级指标得分
    second_level_scores: 二级指标得分
    first_weights: 一级指标权重
    second_weights: 二级指标权重  
    third_weights: 三级指标权重
    top_n: 显示前多少名
    """
    ...
    for i, (idx, row) in enumerate(final_scores.head(top_n).iterrows()):
        anime_name = row['anime']
        ...
        for first_code in config.FIRST_LEVEL_NAMES.keys():
            if first_code in first_level_scores.columns:
                first_scores[config.FIRST_LEVEL_NAMES[first_code]] = \
                    first_level_scores.loc[first_level_scores['anime'] == anime_name, first_code].iloc[0]
    ...
    # 转换权重信息为可保存的格式
    weights_data = []
    for level, weights in weights_report.items():
    ...
    print(f"详细排名报告已保存到 output/detailed_ranking_report.csv")
    print(f"权重报告已保存到 output/weights_report.csv")
    
    # 在控制台输出前10名
    print(f"\n前{min(10, top_n)}名动漫排名:")
    print("=" * 80)
    for i, (idx, row) in enumerate(final_scores.head(10).iterrows()):
        print(f"{i+1:2d}. {row['anime']:20} 综合得分: {row['final_score']:.4f}")
