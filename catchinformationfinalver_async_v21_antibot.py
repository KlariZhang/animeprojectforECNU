...
OUTPUT_CSV = "C:/Code/python/animeproject/fanmade_results_anime_repair.csv"
...
def read_anime_names(csv_path):
    """从CSV文件中读取动漫名称列表"""
    df = pd.read_excel(csv_path, sheet_name="anime")  # 读取指定 sheet
    anime_list = df["anime"].dropna().tolist()  # 提取非空的“movie”列为列表
